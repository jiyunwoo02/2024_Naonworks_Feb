...
intmax_t getPattern() { // 패턴 번호 입력
	...
	signal(SIGINT, handle_signal); // ctrl c 입력 -- 시그널 SIGINT
    ...
    while (1) {
        ...
        if (choice != pattern_triangle && choice != pattern_invert_triangle && 
				...
				choice != pattern_invert_pyramid && choice != pattern_height_triangle && 
				choice != pattern_length_triangle && choice != pattern_random_option) {
            printf("\n1~8 중 다시 입력해주세요.\n");
            continue;
        }
        break;
...
intmax_t getLine(enum pattern_type choice) { // 라인 수 입력
    char buffer[MAX_LINE + 1];
    intmax_t line;
