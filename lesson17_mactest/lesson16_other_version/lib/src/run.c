...
void printStars(enum TYPE pattern, int line_number){
    ...
    switch (pattern)
    {
        ...
        case RANDOM_ASC:
            ...
            break;
        case PYRAMID:
            printPyramid(line_number);
            break;
    };
}

void drawPyramid(int headY, int headX, int arr[][arr_sizeX+1]){
    int cnt=0;
    int nowIdxY=headY;
    int nowIdxX=headX;
    ...
    while(cnt<2){
        arr[nowIdxY+1][--leftIdxX]=1; // 왼 대각
        arr[nowIdxY+1][++rightIdxX]=1; //오른 대각
    ...
    for(int k=0; k<3; k++){
        arr[nowIdxY][++nowIdxX]=1; // 밑변
    }
}


void pyramid(int headY, int headX, int n, int arr[][arr_sizeX+1]){

    if(headX < 0 || headX >arr_sizeX || headY >arr_sizeY){
        return;
    }

    if(n==1){
        arr[headY][headX]=1; // 식에 개체포인터 형식
        drawPyramid(headY, headX, arr);
        return;
    }
    ...
    if(n>3){
        change_sizeX= 5;//pow(2,n-3)+3;
    }

    int change_sizeY = 3;//pow(2,n-2);

    //top
    pyramid(headY,headX, n-1, arr);
    //down_left
    pyramid(headY+change_sizeY, headX-change_sizeX, n-1, arr);
    //down_right
    pyramid(headY+change_sizeY, headX+change_sizeX, n-1, arr);
    return;
}
// 좌표 평면상의 x, y 개념을 사용해서 진행하겠슴다

void printPyramid(int n){

    arr_sizeX= 5;//pow(2, n-1);
    arr_sizeY= 3;//pow(2,n-1);



    arr_sizeX+=1;//pow(2,n-1);

    
    int arr[arr_sizeY+1][arr_sizeX+1];


    for(int i=0; i<arr_sizeY+1; i++){
        for(int j=0; j<arr_sizeX+1; j++){
            arr[i][j]=0;
        }
    }
    int startX=arr_sizeX/2;
    int startY=0;
    pyramid(startY, startX, n, arr);
